...
{  //add some code here to allow user to set compression and framerate?
    ...
    //
    print(" making composite for avi file");
    IJ.run(imp, "Make Composite", ""); //this generates the headless error 
    // because this plugin wants a window to show up.
    // however, it doesn't seem to kill the script
    // imp = IJ.getImage();
    compression="JPEG";
    framerate="7";
    IJ.run(imp, "AVI... ", 
	   "compression=" +compression +" frame=" +framerate +" save=" +outname);
}
...
else if ((outextension==".tif") || (outextension==".tiff"))
    {
            IJ.saveAs(imp, "Tiff", outname);
}
